...
public class MainApp {
    ...
    private DbOutputServicePlugin plugin = null;

    public MainApp() {
        try {
            plugin = new DbOutputServicePlugin();
        } catch (Exception ex) {
            logger.fatal(ex, ex);
        }
    }
    ...
    public final void exec() {
        logger.info("Db output service started");
        try {
            start();
            while (true) {
            ...
            logger.info("db output service stopped...");
            stop();
        } catch (Exception ex) {
            logger.fatal(ex, ex);
        }
        logger.info("db output service stop");
    }

 ...
 */
public class UnitPropertyRecordFactory extends AbstractRecordFactory<UnitPropertyInfoEntity> {
    ...
    public UnitPropertyRecordFactory(Table table, LinkedList<UnitPropertyInfoEntity> entitys) {
        super(table, entitys);
    }
    ...
    @Override
    protected Record createRecord(UnitPropertyInfoEntity entity) {
        ...
        // プロパティ名
        // cells.add(new CellTextField(record, entity.unitPropertyNameProperty()).addStyleClass("ContentTextBox"));
        cells.add(new CellRegexTextField(record, "^.{0,255}$", entity.unitPropertyNameProperty(), CellRegexTextField.RegexType.STRING).addStyleClass("ContentTextBox"));
        // TODO: カスタムフィールド用のEnumから選択できるようにする.デフォルトの値はその他
        // プロパティタイプ
        cells.add(new CellComboBox<>(record, Arrays.asList(CustomPropertyTypeEnum.values()), new CustomPropertyTypeComboBoxCellFactory(), comboCellFactory, entity.unitPropertyTypeProperty()).addStyleClass("ContentComboBox"));
        // プロパティ値
        cells.add(new CellTextField(record, entity.unitPropertyValueProperty()).addStyleClass("ContentTextBox"));
    ...
    public Class getEntityClass() {
        return UnitPropertyInfoEntity.class;
    }

...
public class KanbanCheckerUtils {
     ...
     * 工程カンバンの時間確認
     *
     * @param works ワークフローの工程
     * @param spworks 単一の工程
     * @return
     */
    public static Boolean checkEmptyWorkKanban(List<WorkKanbanInfoEntity> works, List<WorkKanbanInfoEntity> spworks) {
        for (WorkKanbanInfoEntity work : works) {
            if (work.getStartDatetime().after(work.getCompDatetime())) {
                return true;
            }
        }
        for (WorkKanbanInfoEntity work : spworks) {
            if (work.getStartDatetime().after(work.getCompDatetime())) {
                return true;
            }
        }
     ...
     */
    public static Boolean isEmptyKanbanProp(List<KanbanPropertyInfoEntity> entitys) {
        for (KanbanPropertyInfoEntity entity : entitys) {
            entity.updateMember();

...
public class RosterCompoController implements Initializable, ComponentHandler {
    ...
    private final SimpleDateFormat dateCharFormatter = new SimpleDateFormat("yyyy年MM月dd日");
        ...
        this.plansList.setOnMouseClicked((MouseEvent event) -> {
            if (event.getButton() == MouseButton.PRIMARY) {
                if (event.getClickCount() == 2) {
                    this.onPlansUpdate(new ActionEvent());
                } else {
                    // カンバン操作ボタンの有効状態を設定する。
    ...
    private void setCalendar(Calendar calendar) {
        ...
        this.linkTargetYearNow.setText(String.valueOf(calendar.get(Calendar.YEAR)).concat("年"));
        this.linkTargetMonthNow.setText(String.valueOf(calendar.get(Calendar.MONTH) + 1).concat("月"));
     ...
     */
    @FXML
    private void onPlansUpdate(ActionEvent event) {
    ...
    private void setTargetDay(Calendar _calendar, int weekRow, int weekCol) {
        ...
        this.targetDayText.setText(this.dateCharFormatter.format(_calendar.getTime()).concat("の予定"));
        this.targetDay = Calendar.getInstance();

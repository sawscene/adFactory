...
//    private final static String DATETIME_REGEX = "\\d|:|/|-|\\s";
    ...
    public void setArgument(Object argument) {
        ...
        logger.debug(" 組織リスト:" + this.selectOrganizations.size() + " ? "+ this.inputOrganizations.getItems().size());
//        logger.info(" 組織データ:" + this.selectOrganizations.get(0).toString() + " ? "+ this.inputOrganizations.getItems().get(0).toString());
    ...
    private boolean isInputCheck(){
        ...
        if(Objects.nonNull(this.inputStartDate.getValue()) && Objects.nonNull(this.inputStopDate.getValue())){
            ...
            if( dateStop.compareTo(dateStart) < 1 ){
                ...
                // 過去日のチェック
                Date dateNow = org.apache.commons.lang.time.DateUtils.truncate(new Date(), Calendar.DAY_OF_MONTH);
                logger.trace(" 過去日チェック(開始日) C:" + dateNow.compareTo(org.apache.commons.lang.time.DateUtils.truncate(dateStart, Calendar.DAY_OF_MONTH)));
                logger.trace(" 過去日チェック(終了日) C:" + dateNow.compareTo(org.apache.commons.lang.time.DateUtils.truncate(dateStop, Calendar.DAY_OF_MONTH)));
                logger.trace(" 過去日チェック(開始日) A:" + dateNow.after(org.apache.commons.lang.time.DateUtils.truncate(dateStart, Calendar.DAY_OF_MONTH)));
                logger.trace(" 過去日チェック(終了日) A:" + dateNow.after(org.apache.commons.lang.time.DateUtils.truncate(dateStop, Calendar.DAY_OF_MONTH)));
                logger.trace(" 過去日チェック(開始日) B:" + dateNow.before(org.apache.commons.lang.time.DateUtils.truncate(dateStart, Calendar.DAY_OF_MONTH)));
                logger.trace(" 過去日チェック(終了日) B:" + dateNow.before(org.apache.commons.lang.time.DateUtils.truncate(dateStop, Calendar.DAY_OF_MONTH)));
                if( dateNow.after(org.apache.commons.lang.time.DateUtils.truncate(dateStart, Calendar.DAY_OF_MONTH)) && dateNow.after(org.apache.commons.lang.time.DateUtils.truncate(dateStop, Calendar.DAY_OF_MONTH))){
                    logger.trace("日時の過去日");
                    value = false;
                    ProductionNaviUtils.setFieldError(this.inputStartDate);
                    ProductionNaviUtils.setFieldError(this.inputStartTime);

...
-- カンバンビュー
/*
DROP VIEW view_trn_kanban;
CREATE VIEW view_trn_kanban AS
 SELECT * FROM 
  dblink('dbname=adFactoryDB user=postgres password=@dtek1977', 'SELECT kanban_id, kanban_name, fk_workflow_id, start_datetime, comp_datetime, kanban_status, actual_start_datetime, actual_comp_datetime FROM trn_kanban')
  AS t1(kanban_id bigint, 
...
DECLARE
  ...
  query text := 'SELECT kanban_id, kanban_name, fk_workflow_id, start_datetime, comp_datetime, kanban_status, actual_start_datetime, actual_comp_datetime FROM trn_kanban WHERE kanban_id IN (';
...
DECLARE
  ...
  query text := 'SELECT wk.work_kanban_id, wk.fk_kanban_id, wk.fk_work_id, w.work_name, wk.start_datetime, wk.comp_datetime, wk.takt_time, wk.sum_times, wk.work_status, wk.actual_start_datetime, wk.actual_comp_datetime FROM trn_work_kanban wk LEFT JOIN mst_work w ON w.work_id = wk.fk_work_id WHERE wk.fk_kanban_id IN (';
...
BEGIN
  ...
  LOOP
    ...
    IF NOT FOUND THEN EXIT; END IF;
      ...
      fk_kanban_id := rec.fk_kanban_id;
      fk_work_id := rec.fk_work_id;
...
DECLARE
  query text := 'SELECT COUNT(wk.work_status = ''COMPLETION'' or null) / COUNT(wk.work_kanban_id)::float progress_rate FROM trn_work_kanban wk WHERE wk.fk_kanban_id IN (';
  
BEGIN
  query := query || array_to_string(kanban_ids, ',') || ') AND wk.skip_flag = false';
  RETURN (SELECT * FROM dblink('dbname=adFactoryDB user=fujio password=fuji2017', query) AS (progress_rate double precision));
END;
$$ LANGUAGE plpgsql;
...
DECLARE
  ...
  query text := 'SELECT a.fk_kanban_id, a.fk_work_kanban_id, a.actual_status, a.implement_datetime, o.organization_name FROM trn_actual_result a LEFT JOIN mst_organization o ON o.organization_id = a.fk_organization_id WHERE a.fk_work_kanban_id = ';
...
DECLARE
  ...
  conn_name text := 'actual_result_conn';
  cur_name text := 'actual_result_cur';
  query text := 'SELECT a.actual_id, a.fk_kanban_id, a.fk_work_kanban_id, a.actual_status, a.implement_datetime, o.organization_name FROM trn_actual_result a LEFT JOIN mst_organization o ON o.organization_id = a.fk_organization_id WHERE a.fk_kanban_id IN (';
...
-- t_verを更新
UPDATE t_ver SET verno='2' WHERE sid = 1;

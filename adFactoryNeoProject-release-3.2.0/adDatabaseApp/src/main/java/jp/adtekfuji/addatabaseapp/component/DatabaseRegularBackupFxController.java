 ...
 * @author s-maeda
 */
public class DatabaseRegularBackupFxController implements Initializable {

    private final static Logger LOGGER = LogManager.getLogger();
    public final static String ScheduleName = "\"" + PGContents.ADFACTORY_DB + " Regular Backup\"";
    ...
    private DatabaseMainteDayInfoPaneFxController regularBackupDayInfoPane;
            ...
            scheduleTypeCombo.getSelectionModel().selectedItemProperty().addListener((observable, oldValue, newValue) -> {
                if (!newValue.equals(oldValue)) {
                    if (this.scheduleTypes.get(oldValue).equals("Weekly")) {
                        this.daysOfWeek = this.regularBackupDayInfoPane.get();
                    } else if (this.scheduleTypes.get(oldValue).equals("Monthly")) {
                        this.dayOfMonth = this.regularBackupDayInfoPane.get();
                    }
        ...
        try {
            ...
            switch (selectedScheduleType) {
                case "Weekly":
                    ...
                    this.regularBackupDayInfoPane = dayOfWeekCtrl;
                ...
                case "Monthly":
                    ...
                    this.regularBackupDayInfoPane = dayOfMonthCtrl;
                    break;

                default:
                    // その他(「毎日」選択時含む)
                    this.regularBackupDayInfoPane = null;
                    break;
    ...
    public void setDialog(Dialog dialog) {
        this.dialog = dialog;
        this.dialog.getDialogPane().getScene().getWindow().setOnCloseRequest((WindowEvent we) -> this.closeDialog());
    }
     ...
     * 定期実行をスケジュールする
     */
    private void scheduleRegularBackup() {
        LOGGER.info("scheduleRegularBackup start.");
        ...
        try {
            ...
            argList.add("/tr");
            argList.add("\"%ADFACTORY_HOME%\\bin\\adDatabaseApp.exe -update -backup\"");

            argList.add("/sc");
            argList.add(this.scheduleType);

            if (this.scheduleType.equals("Monthly") && this.regularBackupDayInfoPane.get().equals("32")) {
                argList.add("/mo");
                argList.add("LASTDAY");
                ...
                argList.add("/d");
                argList.add(this.regularBackupDayInfoPane.get());
            ...
            LOGGER.info("scheduleRegularBackup end.");
        }
    }
    ...
    public void setCurrentSetting(Boolean enabled, String scheduleType, String startTime, String scheduleDays) {
        try {

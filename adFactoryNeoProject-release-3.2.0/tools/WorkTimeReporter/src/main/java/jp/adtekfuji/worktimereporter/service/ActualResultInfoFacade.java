 ...
 */
package jp.adtekfuji.worktimereporter.service;
...
import jp.adtekfuji.adFactory.entity.actual.IndirectWorkReportOutInfoEntity;
...
public class ActualResultInfoFacade {
    ...
    private final static String INDIRECT_REPORT_OUT_PATH = "/indirect-reportout";
     ...
     * @return 実績出力情報一覧 (エラー発生時はnull)
     */
    public List<IndirectWorkReportOutInfoEntity> indirectWorkReportOutSearch(IndirectWorkSearchCondition condition) {

        logger.info("indirectWorkReportOutSearch start: {}", condition);
        try {
            StringBuilder sb = new StringBuilder();
            sb.append(ACTUAL_PATH);
            sb.append(INDIRECT_REPORT_OUT_PATH);
            sb.append(SEARCH_PATH);
            sb.append(RANGE_PATH);

            return this.restClient.put(sb.toString(), condition, new GenericType<List<IndirectWorkReportOutInfoEntity>>() {
            });
        } catch (Exception ex) {
            logger.fatal(ex, ex);
            return null;
    ...
    public ActualResultEntity find(Long id) {
        ...
        try {
            ...
            return (ActualResultEntity) restClient.find(sb.toString(), ActualResultEntity.class);
        } catch (Exception ex) {
            logger.fatal(ex, ex);
            return null;
        }

 ...
 * @author nar-nakamura
 */
public class WorkPlanColorTextCell extends ListCell<WorkPlanColorTextCellData> {
    ...
    private final ListView<WorkPlanColorTextCellData> listView;
     ...
     * @param listView
     */
    public WorkPlanColorTextCell(ListView<WorkPlanColorTextCellData> listView) {
        super();
        ...
        this.textField.setOnKeyPressed((KeyEvent event) -> {
            if (event.getCode().equals(KeyCode.ENTER)) {
                // 編集を完了する
                WorkPlanColorTextCell.this.commitEdit(new WorkPlanColorTextCellData(WorkPlanColorTextCell.this.textField.getText(), this.textColor));
            }
        });

        this.textField.focusedProperty().addListener((ObservableValue<? extends Boolean> observable, Boolean oldValue, Boolean newValue) -> {
            if (!newValue && WorkPlanColorTextCell.this.isEditing()){
                // 編集をキャンセルする
                WorkPlanColorTextCell.this.cancelEdit();
            }
        });
        ...
        this.deleteButton.setOnAction((ActionEvent event) -> {
            // リストからアイテムを削除する
            Optional<WorkPlanColorTextCellData> target = WorkPlanColorTextCell.this.listView.getItems().stream().filter(p -> text.equals(p.getText())).findFirst();
            if (target.isPresent()) {
                WorkPlanColorTextCell.this.listView.getItems().remove(target.get());
            }
        });
     ...
     */
    @Override
    protected void updateItem(WorkPlanColorTextCellData item, boolean empty) {
     ...
     */
    @Override
    public void commitEdit(WorkPlanColorTextCellData newValue) {
        if (this.isEditing()) {

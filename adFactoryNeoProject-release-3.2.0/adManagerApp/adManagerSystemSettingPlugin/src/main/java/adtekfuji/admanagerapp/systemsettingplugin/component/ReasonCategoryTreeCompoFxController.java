 ...
 * @author hato
 */
@FxComponent(id = "ReasonCategoryTreeCompo",
        fxmlPath = "/fxml/admanagersystemsettingplugin/reason_category_tree_compo.fxml")
public class ReasonCategoryTreeCompoFxController implements Initializable, ArgumentDelivery {
    ...
    @FXML
    private TreeView<ReasonCategoryInfoEntity> categoryTree;
    @FXML
    ...
    public void setArgument(Object argument) {
        if (argument instanceof TreeDialogEntity) {
            ...
            categoryTree.getSelectionModel().selectedItemProperty().addListener(
                    (ObservableValue<? extends TreeItem<ReasonCategoryInfoEntity>> observable,
                            TreeItem<ReasonCategoryInfoEntity> oldValue, TreeItem<ReasonCategoryInfoEntity> newValue)
                    -> {
                boolean isSet = false;
                ...
                } else // データの場合は、ルートには移動不可
                {
                    if (Objects.nonNull(newValue) && newValue.getValue().getId() != ROOT_ID) {
                        isSet = true;
                    }
                }

                if (isSet) {
                    selectedItemName.setText(newValue.getValue().getReasonCategoryName());
            ...
            Platform.runLater(() -> {
                ...
                categoryTree.rootProperty()
                        .setValue((TreeItem<ReasonCategoryInfoEntity>) treeDialogEntity.getTreeRootItem());
                reRenderTree();
            });
        }
    }
    ...
    private void reRenderTree() {
        categoryTree.setCellFactory((TreeView<ReasonCategoryInfoEntity> p) -> new ReasonCategoryTreeCell());
    }
}

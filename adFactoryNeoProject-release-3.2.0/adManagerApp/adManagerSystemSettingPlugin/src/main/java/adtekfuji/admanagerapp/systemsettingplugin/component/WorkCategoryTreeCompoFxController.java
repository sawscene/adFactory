 ...
 * @author ta.ito
 */
@FxComponent(id = "WorkCategoryTreeCompo",
        fxmlPath = "/fxml/admanagersystemsettingplugin/work_category_tree_compo.fxml")
public class WorkCategoryTreeCompoFxController implements Initializable, ArgumentDelivery {
    ...
    @FXML
    private TreeView<WorkCategoryInfoEntity> categoryTree;
    @FXML
    ...
    public void setArgument(Object argument) {
        if (argument instanceof TreeDialogEntity) {
            ...
            categoryTree.getSelectionModel().selectedItemProperty().addListener(
                    (ObservableValue<? extends TreeItem<WorkCategoryInfoEntity>> observable,
                            TreeItem<WorkCategoryInfoEntity> oldValue, TreeItem<WorkCategoryInfoEntity> newValue)
                    -> {
                boolean isSet = false;
                ...
                {
                    if (Objects.nonNull(newValue) && newValue.getValue().getWorkCategoryId() != ROOT_ID) {
                        isSet = true;
                    }
                }

                if (isSet) {
                    selectedItemName.setText(newValue.getValue().getWorkCategoryName());
            ...
            Platform.runLater(() -> {
                ...
                categoryTree.rootProperty()
                        .setValue((TreeItem<WorkCategoryInfoEntity>) treeDialogEntity.getTreeRootItem());
                reRenderTree();
            });
        }
    }
    ...
    private void reRenderTree() {
        categoryTree.setCellFactory((TreeView<WorkCategoryInfoEntity> p) -> new WorkCategoryTreeCell());
    }
}

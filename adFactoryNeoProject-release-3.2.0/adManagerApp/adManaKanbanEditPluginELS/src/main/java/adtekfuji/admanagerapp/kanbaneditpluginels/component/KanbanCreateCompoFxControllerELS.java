...
import adtekfuji.admanagerapp.kanbaneditpluginels.externalio.KanbanBaseInfoAccessELS;
...
import adtekfuji.admanagerapp.kanbaneditpluginels.controls.ColorTextCellData;
...
import adtekfuji.admanagerapp.kanbaneditpluginels.controls.ColorTextCell;
 ...
 * @author s-heya
 */
@FxComponent(id = "KanbanCreateCompoELS", fxmlPath = "/fxml/admanakanbaneditpluginels/kanban_create_compo_els.fxml")
public class KanbanCreateCompoFxControllerELS implements Initializable, SerialCommunicationListener, ArgumentDelivery, ListChangeListener  {
    ...
    private final KanbanBaseInfoAccessELS kanbanBaseInfoAccess = new KanbanBaseInfoAccessELS();// カンバン基本情報取得用クラス
    ...
    public void initialize(URL url, ResourceBundle rb) {
        try {
            ...
            // 作業指示コードラベル
            String codeLabelText = KanbanEditConfig.getIndtructionCodeLabelText();
            if (codeLabelText.isEmpty()) {
                codeLabelText = LocaleUtils.getString("key.InstructionCode");
                KanbanEditConfig.setIndtructionCodeLabelText(codeLabelText);
            }
    ...
    private void createKanbanThread() throws Exception {
        ...
        Optional<KanbanPropertyInfoEntity> porderProp = this.propertyList.stream().filter(prop -> Objects.equals(prop.getKanbanPropertyName(), "注番")).findFirst();
        ...
        // シリアル番号毎にカンバンを作成する。
        for (ColorTextCellData listItem : serialList) {
            ...
            try {
                ...
                workflowProcess.setBaseTime(kanban, breaktimeList, new Date());

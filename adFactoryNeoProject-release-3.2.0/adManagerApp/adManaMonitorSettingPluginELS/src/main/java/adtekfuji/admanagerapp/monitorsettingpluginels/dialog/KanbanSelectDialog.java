 ...
 */
package adtekfuji.admanagerapp.monitorsettingpluginels.dialog;
...
public class KanbanSelectDialog {
    ...
    public static List<KanbanInfoEntity> getKanbans(List<Long> targets) {
        return kanbanInfoFacade.find(targets);
    }
     ...
     * @param selectedDate
     */
    public static ButtonType showDialog(ActionEvent event, Date selectedDate) {
        ...
        try {
            ...
            if (Objects.nonNull(kanbans)) {
                selectDialogEntity.kanbans(kanbans, null, false);
            }
            ...
            ret = sc.showComponentDialog(LocaleUtils.getString("key.Kanban"), "KanbanSelectionCompo", selectDialogEntity, (Stage) ((Node) event.getSource()).getScene().getWindow());

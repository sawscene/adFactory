...
/**
 * ロットトレース(製品)画面 コントローラー
 *
 * @author nar-nakamura
 */
@FxComponent(id = "LotTraceKanban", fxmlPath = "/fxml/warehouseplugin/lot_trace_kanban.fxml")
public class LotTraceKanbanController implements Initializable, ComponentHandler {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        this.mainTable.setRowFactory(value -> {
            ...
            this.mainTable.getSelectionModel().getSelectedItems().addListener((Change<? extends TreeItem<TrnLotTraceInfo>> item) -> {
                if (row.isFocused()) {
                    row.setStyle(null);
                } else {
        ...
        this.mainTable.setColumnResizePolicy(TreeTableView.CONSTRAINED_RESIZE_POLICY);
    ...
    private void loadSearchConditions() {
        try {
            ...
            String prefix = LotTraceKanbanController.class.getName() + ".";
        ...
        try {
            ...
            Task task = new Task<List<TrnLotTraceInfo>>() {
                ...
                protected List<TrnLotTraceInfo> call() throws Exception {
                    ...
                    // ソートする。
                    Collections.sort(result, LotTraceComparators.lotTraceModelNameComparator
    ...
    private void updateMainTable() {
        ...
        try {
            ...
            for (TrnLotTraceInfo lotTrace : lotTraceList) {
                if (!Objects.equals(parent.getModelName(), lotTrace.getModelName())
                        || !parent.isParent()) {
                    ...
                    // 親アイテム(モデル名)
                    String modelName = lotTrace.getModelName();
                    if (StringUtils.isEmpty(modelName)) {
                        modelName = LocaleUtils.getString("key.Other");
                    }

                    parent = TrnLotTraceInfo.createKanbanItem(null, modelName, lotTrace.getModelName());
                    parentItem = new TreeItem(parent);
                    parentItem.setExpanded(true);
                }

                if (Objects.equals(item.getModelName(), lotTrace.getModelName())
                        && Objects.equals(item.getKanbanId(), lotTrace.getKanbanId())) {
                    continue;
                }

                item = TrnLotTraceInfo.createKanbanItem(lotTrace.getKanbanId(), lotTrace.getKanbanName(), lotTrace.getModelName());

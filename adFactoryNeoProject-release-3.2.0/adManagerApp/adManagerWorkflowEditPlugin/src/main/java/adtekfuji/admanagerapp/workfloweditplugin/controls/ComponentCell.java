 ...
 * @author itage
 */
public class ComponentCell extends ListCell<TraceCustomEntity> {
    ...
    private final TextField ruleTextField;
    ...
    public ComponentCell() {
        ...
        this.ruleTextField.setPrefWidth(170.0);
        ...
        // 入力規則
        this.ruleTextField.focusedProperty().addListener((ObservableValue<? extends Boolean> observable, Boolean oldValue, Boolean newValue) -> {
            if (!newValue && (oldValue != newValue)) {
                String newText  = this.ruleTextField.getText();
                if (Objects.isNull(newText)) {
                    newText = "";
                }

                // 入力された値をentityにセットする。
                this.getItem().setRule(newText);
            }
        });
        ...
        this.valueTextField.focusedProperty().addListener((ObservableValue<? extends Boolean> observable, Boolean oldValue, Boolean newValue) -> {
            if (!newValue && (oldValue != newValue)) {
                ...
                // 重複確認
                long count = this.getListView().getItems().stream().filter(p -> p.getValue().equals(newValueText) && !p.equals(this.getItem())).count();
                if (count == 0) {
                    // 入力された値をentityにセットする。
                    this.getItem().setValue(newValueText);
                } else {
                    // 重複する項目名が存在する場合、警告して元の値に戻す。
                    String msg = String.format(LocaleUtils.getString("key.AddErrAlreadyMessage"), newValueText);
        ...
        this.hbox.getChildren().addAll(this.nameTextField, this.ruleTextField, this.valueTextField, this.pane, this.deleteButton);

...
rem if DEFINED JRE_HOME goto gotJreHome
...
rem for /f tokens^=2^ delims^=.-_^" %%j in ('"%JRE_HOME%\bin\java.exe" -fullversion 2^>^&1') do set "JAVA_MAJOR_VERSION=%%j"
rem goto okJavaHome
:gotJdkHome
...
rem if not "%JRE_HOME%" == "" goto okJavaHome
if %JAVA_MAJOR_VERSION% lss 11 (
...
set SERVICE_NAME=TomEE_10
set PR_DISPLAYNAME=Apache TomEE 10.0.1
...
if %JAVA_MAJOR_VERSION% lss 11 (
    "%EXECUTABLE%" //US//%SERVICE_NAME% ^
    	++JvmOptions "-javaagent:%CATALINA_HOME%\lib\openejb-javaagent.jar;-Dcatalina.base=%CATALINA_BASE%;-Dcatalina.home=%CATALINA_HOME%;-Djava.endorsed.dirs=%CATALINA_HOME%\endorsed;-Djava.net.preferIPv4Stack=true;-Dfile.encoding=UTF-8"
) else (
    "%EXECUTABLE%" //US//%SERVICE_NAME% ^
	    ++JvmOptions "-javaagent:%CATALINA_HOME%\lib\openejb-javaagent.jar;-Dcatalina.base=%CATALINA_BASE%;-Dcatalina.home=%CATALINA_HOME%;-Djava.net.preferIPv4Stack=true;-Dfile.encoding=UTF-8"
)
...
rem ログオンユーザーアカウントをLocal ServiceからLocal Systemに変更
rem (Windows Serverでは「sc」は「Set-Content」として扱われるので、「sc.exe」とexeを付けてSet-Contentと異なるものであることを明示する)

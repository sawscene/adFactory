...
CREATE VIEW view_work_report AS 
  ...
  SELECT
    ...
    -- 工数(ms)
    SUM(((act.work_time * ((job->'LVOL')::int + (job->'DEFECT')::int)) / kan.lvol_sum)) work_time,
    -- 工程順ID
    act.workflow_id,
    -- カンバン名
    kan.kanban_name,
    ...
    kan.model_name,
    -- 実績数
    SUM(((act.comp_num * ((job->'LVOL')::int + (job->'DEFECT')::int)) / kan.lvol_sum)) actual_num,
    -- 作業種別の順
    1 AS work_type_order,
    -- 製造番号
    kan.production_number
  ...
  LEFT JOIN (
    SELECT
      ...
      kanban_subname,
      service_info,
      CASE WHEN (SUM((job->'LVOL')::int + (job->'DEFECT')::int)) = 0
        THEN 1
        ELSE (SUM((job->'LVOL')::int + (job->'DEFECT')::int))
        END lvol_sum
    FROM trn_kanban

...
create table trn_lot_trace (
  ...
  , constraint trn_lot_trace_pk primary key (delivery_no,item_no,material_no)
) ;

comment on table trn_lot_trace is 'ロットトレース';
comment on column trn_lot_trace.delivery_no is '出庫番号';
comment on column trn_lot_trace.item_no is '明細番号';
comment on column trn_lot_trace.material_no is '資材番号';
comment on column trn_lot_trace.serial_no is '製造番号:供給元(メーカー、サプライヤー)が採番したロット番号又はシリアル番号';
comment on column trn_lot_trace.parts_no is '部品番号:仕入元が独自に採番した番号';
comment on column trn_lot_trace.trace_num is '数量';
comment on column trn_lot_trace.person_no is '社員番号';
comment on column trn_lot_trace.kanban_id is 'カンバンID';
comment on column trn_lot_trace.work_kanban_id is '工程カンバンID';
comment on column trn_lot_trace.ver_info is '排他用バージョン';
comment on column trn_lot_trace.confirm is '確認';
comment on column trn_lot_trace.create_date is '作成日時';
comment on column trn_lot_trace.update_date is '更新日時';

-- 「資材情報」テーブルに「部品番号」を追加する。
ALTER TABLE trn_material ADD parts_no character varying(32); -- 部品番号
comment on column trn_material.parts_no is '部品番号:仕入元が独自に採番した番号';


-- 「部品マスタ」テーブルに「図番」を追加する。
ALTER TABLE mst_product ADD figure_no character varying(32); -- 図番
comment on column mst_product.figure_no is '図番';


-- ユニークインデックスを追加する。
create unique index mst_product_IDX02
  on mst_product(figure_no);


-- コメントを変更
comment on column trn_delivery.order_no is '製造オーダー番号:工事No';
comment on column trn_delivery_item.order_no is '製造オーダー番号';
comment on column trn_material.order_no is '製造オーダー番号:製番';
comment on column trn_material.serial_no is '製造番号:供給元(メーカー、サプライヤー)が採番したロット番号又はシリアル番号';


-- 作業日報ビュー (直接作業・間接作業・中断時間) の更新
-- "integer out of range"の対策のため
...
CREATE VIEW view_work_report AS 
  ...
  SELECT
    ...
    -- 工数(ms)
    SUM(((act.work_time * ((job->'LVOL')::bigint + (job->'DEFECT')::bigint)) / kan.lvol_sum)) work_time,
    -- 工程順ID
    act.workflow_id,
    -- カンバン名
    kan.kanban_name,
    ...
    kan.model_name,
    -- 実績数
    SUM(((act.comp_num * ((job->'LVOL')::bigint + (job->'DEFECT')::bigint)) / kan.lvol_sum)) actual_num,
    -- 作業種別の順
    1 AS work_type_order,
    -- 製造番号
    kan.production_number
  ...
  LEFT JOIN (
    SELECT
      ...
      kanban_subname,
      service_info,
      CASE WHEN (SUM((job->'LVOL')::bigint + (job->'DEFECT')::bigint)) = 0
        THEN 1
        ELSE (SUM((job->'LVOL')::bigint + (job->'DEFECT')::bigint))
        END lvol_sum
    FROM trn_kanban
    ...
    GROUP BY
      ...
      kanban_subname,
      service_info
  ) kan ON kan.kanban_id = act.kanban_id

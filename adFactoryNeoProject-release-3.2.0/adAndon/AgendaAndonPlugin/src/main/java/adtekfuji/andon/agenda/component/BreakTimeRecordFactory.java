 ...
 * @author e.mori
 */
public class BreakTimeRecordFactory extends AbstractRecordFactory<BreakTime> {
    ...
    public BreakTimeRecordFactory(Table table, List<BreakTimeInfoEntity> breaktimes, LinkedList<BreakTime> entities) {
        super(table, entities);
        this.breaktimes = breaktimes;
    }

    @Override
    protected Record createRecord(BreakTime entity) {
        ...
        if (breaktimes.size() >= 1) {
            ...
            CellComboBox cell = new CellComboBox<>(record, array, new BreakTimeComboBoxCellFactory(), comboCellFactory, entity.idProperty());
            cell.actionListner(new ChangeListener<Number>() {
                @Override
                public void changed(ObservableValue<? extends Number> ov, Number oldValue, Number newValue) {
                    entity.setId((Long) newValue);
                }
            });

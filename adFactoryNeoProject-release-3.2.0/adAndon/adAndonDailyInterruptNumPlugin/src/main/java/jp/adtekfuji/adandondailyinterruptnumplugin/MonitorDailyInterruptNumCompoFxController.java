package jp.adtekfuji.adandondailyinterruptnumplugin;
...
@AndonComponent(title = "中断理由カウントフレーム")
@FxComponent(id = "DailyInterruptNum", fxmlPath = "/fxml/daily_interruptnum_compo.fxml")
public class MonitorDailyInterruptNumCompoFxController implements Initializable, AdAndonComponentInterface {
    ...
    public void updateDisplay(Object msg) {
        if (msg instanceof ActualNoticeCommand) {
            ...
            // 工程カンバンステータス
            if (!KanbanStatusEnum.SUSPEND.equals(command.getWorkKanbanStatus())) {
                return;
            }
    ...
    private void readTask(Long monitorId) {
        Task task = new Task<Void>() {
            ...
            protected Void call() throws Exception {
                try {
                    synchronized (lock) {
                        reasonNumInfos = andonLineMonitorFacade.getDailyInterruptReasonInfo(monitorId);
                    }
                } catch (Exception ex) {
                    logger.fatal(ex, ex);
                }
